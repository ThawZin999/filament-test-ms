@props(['paginator', 'pageOptions' => [], 'currentPageOptionProperty' => 'tableRecordsPerPage'])
...
<nav aria-label="{{ __('filament::components/pagination.label') }}" role="navigation"
    ...
    @if ($paginator->hasMorePages())
        ...
        <x-filament::button color="gray" rel="next" :wire:click="$wireClickAction"
            :wire:key="$this->getId() . '.pagination.next'" class="fi-pagination-next-btn col-start-3 justify-self-end">
            ...
            @foreach ($paginator->render()->offsetGet('elements') as $element)
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        <x-filament::pagination.item :active="$page === $paginator->currentPage()" :aria-label="trans_choice('filament::components/pagination.actions.go_to_page.label', $page, [
                            'page' => $page,
                        ])" :label="$page"
